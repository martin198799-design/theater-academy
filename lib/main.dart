...
class _DepartmentDetailScreenState extends State<DepartmentDetailScreen> {
  ...
  void initState() {
    ...
    _geminiMessages = [
      {
        'sender': 'ai',
        'text': 'أهلاً بك يا فنان في ${widget.department['title']}. أنا مساعدك الذكي؛ اطرح أي سؤال فني وسأقوم بتحليله والإجابة عليه فوراً!'
      }
    ];
  }

  // محرك ذكي متطور وموسع يحلل أي كلمة مفتاحية بدقة لضمان عدم تكرار الإجابات العشوائية
  String _generateSmartAnswer(String prompt) {
    String q = prompt.trim().toLowerCase();
    
    if (q.contains('ادوات') || q.contains('أدوات') || q.contains('ممثل')) {
      return 'أدوات الممثل الأساسية ترتكز على ثلاثة محاور:\n1. الجسد: أداة التعبير الحركي والرشاقة والمرونة على الخشبة.\n2. الصوت: النبرات، الطبقات، والقدرة على إيصال الحوار بوضوح.\n3. النفس والذاكرة الانفعالية: استدعاء المشاعر الحقيقية وتوظيفها لخدمة الشخصية.';
    } else if (q.contains('تمرين') || q.contains('حركه') || q.contains('حركة') || q.contains('تدريب')) {
      return 'التمارين الجسدية والحركية تهدف إلى كسر التشنج وتحرير طاقة الممثل، وتشمل:\n- تمارين الوعي بالمكان وتوزيع الثقل.\n- تمارين الإيقاع والتحكم في التنفس.\n- التعبير الحركي الصامت (پانتومايم) لربط الفكرة بالجسد.';
    } else if (q.contains('مخرج') || q.contains('إخراج')) {
      return 'المخرج المسرحي هو صاحب الرؤية البصرية والفكريّة. يقوم بتفكيك النص، رسم حركة الممثلين (الميزانسين)، وتوجيه الإضاءة والديكور لخلق وحدة متناغمة للعرض.';
    } else if (q.contains('نص') || q.contains('مسرحية') || q.contains('يوناني') || q.contains('اغريقي')) {
      return 'النصوص المسرحية الكلاسيكية والحديثة تمثل وعاء الفكرة الدرامية. تتميز بالصراع المتصاعد، الحبكة المحكمة، والانقسام إلى فصول تقود المتفرج نحو الذروة والحل.';
    } else if (q.contains('كيف') || q.contains('طريقة') || q.contains('يسل')) {
      return 'للوصول إلى العمق الفني والتجسيد الصادق، يتطلب الأمر: دراسة خلفية الشخصية، التدريب المستمر للجسد والصوت، والتحضير الذهني العميق قبل صعود الخشبة.';
    } else {
      return 'سؤال عميق ومهم جداً حول ($prompt). في المنهج الأكاديمي المسرحي، نتعامل مع هذه النقطة عبر تحليل الصراع الدرامي، دراسة الأبعاد النفسية، وتوظيف أدوات الممثل والسينوغرافيا لخلق تأثير قوي لدى المتفرج.';
    }
  }
  ...
  Future<void> _askGeminiAssistant(String prompt) async {
    ...
    String reply = _generateSmartAnswer(currentPrompt);
