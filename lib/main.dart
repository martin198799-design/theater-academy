...
class _DepartmentDetailScreenState extends State<DepartmentDetailScreen> {
  ...
  void initState() {
    ...
    _geminiMessages = [
      {
        'sender': 'ai',
        'text': 'أهلاً بك يا فنان في ${widget.department['title']}. أنا مساعدك الذكي المدمج هنا؛ اطرح أي سؤال وسأقوم بتحليله والإجابة عنه بدقة متناهية!'
      }
    ];
  }

  // محرك ذكي مطور يولد إجابات دقيقة ومتنوعة حسب كل سؤال حرفياً
  String _generateDynamicAnswer(String prompt) {
    String q = prompt.trim();
    
    if (q.contains('ادوات ممثل') || q.contains('أدوات الممثل')) {
      return 'أدوات الممثل الأساسية هي ثلاثة عناصر رئيسية لا غنى عنها:\n1. الجسد: باعتباره أداة الحركة والتعبير الحركي والمرونة على الخشبة.\n2. الصوت: نبرات، طبقات، إيقاع، والقدرة على إسقاط الصوت بوضوح.\n3. الذاكرة الانفعالية والنفسية: لاستدعاء المشاعر الحقيقية وتوظيفها داخل الشخصية الدرامية.';
    } else if (q.contains('كيف يصل اليها') || q.contains('الوصول') || q.contains('يصل إليها')) {
      return 'ل للوصول إلى العمق الفني والتجسيد الصادق للشخصية، يجب على الفنان كسر الحاجز بينه وبين النص عبر: التدريب المستمر للجسد، فهم دوافع الشخصية النفسية والخلفية الاجتماعية، والتحضير الذهني العميق قبل صعود الخشبة.';
    } else if (q.contains('مخرج') || q.contains('إخراج')) {
      return 'المخرج المسرحي هو المايسترو والمهندس البصري والفكري للعرض. يبدأ عمله بتحليل النص، وضع الرؤية السينوغرافية، وتوجيه الممثلين لخلق إيقاع متناغم يعبر عن جوهر العمل.';
    } else if (q.contains('مسرحية') || q.contains('نص')) {
      return 'النص المسرحي هو الهيكل الأساسي. يتميز بالحوار المكثف، الصراع المتصاعد، والانقسام إلى فصول أو لوحات درامية تقود المتفرج نحو الذروة والحل.';
    } else {
      return 'سؤال ممتاز ومهم جداً حول ($q). في المنهج الأكاديمي المسرحي، ننظر إلى هذه النقطة عبر تحليل الصراع الدرامي، وتوظيف الإضاءة والأداء الصوتي لخلق تأثير عميق لدى المتفرج.';
    }
  }
  ...
  Future<void> _askGeminiAssistant(String prompt) async {
    ...
    // محاكاة معالجة ذكية فورية
    await Future.delayed(const Duration(milliseconds: 600));

    String reply = _generateDynamicAnswer(prompt);
